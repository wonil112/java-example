...
public class MyStorage {
    ...
    static void prod_input(Scanner s) {
        ...
        // TODO:
        if (products[0] != "등록 가능" && products[1] != "등록 가능" && products[2] != "등록 가능"
                && products[3] != "등록 가능" && products[4] != "등록 가능") {
            System.out.println("[System] 제품이 전부 등록되어 있어 추가적인 등록이 불가능합니다.");
            return;
        }
...
//        if (products[0] == "등록 가능") {
//            System.out.println("[System] 등록된 제품이 없습니다.");
//            return;
//        }
        System.out.println("[System] 제품 등록 취소를 원하는 제품명을 입력하세요 : ");
        String productName = s.nextLine();
        for (int i = 0; i < products.length; i++) {
            if (Objects.equals(products[i], productName)) {
                products[i] = "등록 가능";
                counts[i] = 0;
                break;
            }
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        int cnt = Integer.parseInt(inputNumber);
//        int cnt = Integer.parseInt(s.nextLine());  // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        if(cnt > counts[foundIdx]) {
            System.out.println("[Error] 현재 보유량보다 많은 수량을 출고할 수 없습니다.");
            return;
        }
        counts[foundIdx] -= cnt;
        ...
        System.out.println("[Clear] 정상적으로 제품의 수량 출고가 완료되었습니다.");
    }
